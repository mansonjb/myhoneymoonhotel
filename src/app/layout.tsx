...
const DEST_META: Record<string, { region: string; country: string }> = {
  ...
  'belize':            { region: 'Caribbean & Americas', country: 'Belize' },
  'saint-vincent-grenadines': { region: 'Caribbean & Americas', country: 'St. Vincent & the Grenadines' },
  'dominican-republic': { region: 'Caribbean & Americas', country: 'Dominican Republic' },
  // Europe
  ...
  'venice':     { region: 'Europe', country: 'Italy' },
  'puglia':     { region: 'Europe', country: 'Italy' },
  'capri':      { region: 'Europe', country: 'Italy' },
  ...
  'cartagena':  { region: 'South America', country: 'Colombia' },
  // North America
...
export default async function RootLayout({ children }: { children: React.ReactNode }) {
  // Detect locale from pathname (set by middleware as `x-pathname`).
  const h = await headers()
  const pathname = h.get('x-pathname') ?? '/'
  const firstSegment = pathname.split('/').filter(Boolean)[0] ?? ''
  const locale: Locale = firstSegment === 'es' ? 'es' : firstSegment === 'pt' ? 'pt' : 'en'
  const m = getMessages(locale)
  const localePrefix = locale === 'en' ? '' : `/${locale}`
  const lp = (p: string) => `${localePrefix}${p}`
  ...
  return (
    <html lang={HTML_LANG[locale]} className={`${geist.variable} ${instrumentSerif.variable}`}>
      <head>
      ...
      <body className="bg-white text-zinc-900 antialiased">
        ...
        <header className="fixed top-0 left-0 right-0 z-50 bg-white/90 backdrop-blur-md border-b border-zinc-100">
          <div className="max-w-7xl mx-auto px-6 h-[60px] flex items-center justify-between relative">
            <Link href={lp('/')} className="flex items-center gap-1.5">
          ...
          <div className="max-w-7xl mx-auto px-6 py-16">
            <div className="flex flex-col lg:flex-row justify-between gap-12 mb-12">
              ...
              <div className="max-w-xs">
                ...
                <p className="text-zinc-400 text-sm leading-relaxed">{m['brand.tagline'] ?? 'The honest guide to honeymoon hotels. Every property scored on 9 romance-specific criteria — never paid placement.'}</p>
              </div>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-8">
                <div>
                  ...
                  <ul className="space-y-2.5 text-sm text-zinc-500">
                    {[['Maldives','maldives'],['Bora Bora','bora-bora'],['St. Lucia','st-lucia'],['Turks & Caicos','turks-and-caicos'],['Santorini','santorini'],['Bali','bali']].map(([l,s])=>(
                      <li key={s}><Link href={lp(`/destinations/${s}`)} className="hover:text-zinc-900 transition-colors">{l}</Link></li>
                    ))}
                  </ul>
                </div>
                <div>
                  <h3 className="text-xs font-semibold uppercase tracking-widest text-zinc-400 mb-4">{m['footer.experiences'] ?? 'Experiences'}</h3>
                  <ul className="space-y-2.5 text-sm text-zinc-500">
                ...
                <div>
                  <h3 className="text-xs font-semibold uppercase tracking-widest text-zinc-400 mb-4">{m['footer.about'] ?? 'About'}</h3>
                  <ul className="space-y-2.5 text-sm text-zinc-500">
                    <li><Link href={lp('/about')} className="hover:text-zinc-900 transition-colors">{m['footer.ourStory'] ?? 'Our story'}</Link></li>
                    <li><Link href={lp('/methodology')} className="hover:text-zinc-900 transition-colors">Methodology</Link></li>
                    ...
                    <li><Link href={lp('/terms')} className="hover:text-zinc-900 transition-colors">{m['footer.terms'] ?? 'Terms'}</Link></li>
                  </ul>
                </div>
              </div>
            </div>
            ...
            <div className="border-t border-zinc-100 pt-8 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 text-xs text-zinc-400">
              <p>{m['footer.copyright'] ?? '© 2026 My Honeymoon Hotel.'}</p>
              {locale === 'en' ? (
                ...
                <p>{m['footer.affiliate'] ?? 'Affiliate links via Stay22 — commission at no cost to you, never influences scores.'}</p>
              )}
            </div>
          </div>
        </footer>
