...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return buildDestinationMonthMetadata(country, month, 'fr')
}

export default async function DestinationMonthPageFR({ params }: Props) {
  const { country, month } = await params
  return renderDestinationMonthPage(country, month, 'fr')
}
